...
{
    return fabs(a.x - b.x) > k_epsilon ||
        fabs(a.y - b.y) > k_epsilon ||
        fabs(a.z - b.z) > k_epsilon;
}

bool operator !=(const Face& a, const Face& b)
{
    return a.indexX != b.indexX || a.indexY != b.indexY || a.indexZ != b.indexZ;
}
...
{
    os << "(" << f.indexX << ", " << f.indexY << ", " << f.indexZ << ")";
    return os;
}

bool CheckVertices(const ObjFile& obj, const std::vector<Vertex>& expectedVertices)
{
    const auto& vertices = obj.GetVertices();

    if (vertices.size() != expectedVertices.size())
    {
        std::cout << "Error: Incorrect number of vertices. Expected: " << expectedVertices.size()
            << " Received: " << vertices.size() << std::endl;
        return false;
    }

    bool error = false;

    for (int index = 0, count = expectedVertices.size(); index < count; ++index)
    {
        if (expectedVertices[index] != vertices[index])
        {
            std::cout << "Error: Vertex " << index << " does not match!" << std::endl
                << "  Expected: " << expectedVertices[index] << std::endl
                << "  Received: " << vertices[index] << std::endl;
            error = true;
        }
    }
...
bool CheckFaces(const ObjFile& obj, const std::vector<Face>& expectedFaces)
{
    const auto& faces = obj.GetFaces();

    if (faces.size() != expectedFaces.size())
    {
        std::cout << "Error: Incorrect number of faces. Expected: " << expectedFaces.size()
            << " Received: " << faces.size() << std::endl;
        return false;
    }

    bool error = false;

    for (int index = 0, count = expectedFaces.size(); index < count; ++index)
    {
        if (expectedFaces[index] != faces[index])
        {
            std::cout << "Error: Face " << index << " does not match!" << std::endl
                << "  Expected: " << expectedFaces[index] << std::endl
                << "  Received: " << faces[index] << std::endl;
            error = true;
        }
    }
...
void LogBeginTesting(const char* filename)
{
    std::cout << "--- " << filename << ": Beginning testing ---" << std::endl;
}

void LogFinishedTesting(const char* filename, bool success)
{
    if (success)
    {
        std::cout << "--- " << filename << ": SUCCEEDED ---" << std::endl << std::endl;
    }
    else
    {
        std::cout << "--- " << filename << ": FAILED ---" << std::endl << std::endl;
    }
}

bool TestObjFile(const char* filename, const std::vector<Vertex>& expectedVertices,
    const std::vector<Face>& expectedFaces)
{
    LogBeginTesting(filename);
	...
	{
		error |= !CheckVertices(obj, expectedVertices);
		error |= !CheckFaces(obj, expectedFaces);
	}
	else
    {
        std::cout << "Error: Load() indicated failure!" << std::endl;
		error = true;
    }

    LogFinishedTesting(filename, !error);
...
bool TestObjFileWithCube()
{
    ...
    return TestObjFile("cube.obj", expectedVertices, expectedFaces);
}
...
bool TestObjFileWithSuzanne()
{
    const std::vector<Vertex> expectedVertices =
    {
        ...
        { -0.203125, 0.093750, 0.742188 },
        { 0.156250, 0.054687, 0.648438 },
        { -0.156250, 0.054687, 0.648438 },
        { 0.078125, 0.242187, 0.656250 },
        { -0.078125, 0.242187, 0.656250 },
        { 0.140625, 0.242187, 0.742188 },
        { -0.140625, 0.242187, 0.742188 },
        { 0.242188, 0.242187, 0.796875 },
        { -0.242188, 0.242187, 0.796875 },
        ...
        { -0.546875, 0.437500, 0.578125 },
        { 0.625000, 0.242187, 0.562500 },
        { -0.625000, 0.242187, 0.562500 },
        { 0.562500, 0.242187, 0.671875 },
        { -0.562500, 0.242187, 0.671875 },
        { 0.468750, 0.242187, 0.757812 },
        { -0.468750, 0.242187, 0.757812 },
        { 0.476562, 0.242187, 0.773438 },
        { -0.476562, 0.242187, 0.773438 },
        { 0.445312, 0.335937, 0.781250 },
        { -0.445312, 0.335937, 0.781250 },
        { 0.351562, 0.375000, 0.804688 },
        { -0.351562, 0.375000, 0.804688 },
        { 0.265625, 0.335937, 0.820312 },
        { -0.265625, 0.335937, 0.820312 },
        { 0.226562, 0.242187, 0.820312 },
        { -0.226562, 0.242187, 0.820312 },
        { 0.265625, 0.156250, 0.820312 },
        { -0.265625, 0.156250, 0.820312 },
        { 0.351562, 0.242187, 0.828125 },
        { -0.351562, 0.242187, 0.828125 },
        { 0.351562, 0.117187, 0.804688 },
        { -0.351562, 0.117187, 0.804688 },
        { 0.445312, 0.156250, 0.781250 },
        { -0.445312, 0.156250, 0.781250 },
        { 0.000000, 0.429687, 0.742188 },
        { 0.000000, 0.351562, 0.820312 },
        { 0.000000, -0.679688, 0.734375 },
        { 0.000000, -0.320313, 0.781250 },
        ...
        { 0.000000, 0.070313, -0.828125 },
        { 0.000000, -0.382812, -0.351563 },
        ...
        { -0.367188, -0.890625, 0.531250 },
        { 0.328125, -0.945312, 0.523437 },
        { -0.328125, -0.945312, 0.523437 },
        { 0.179688, -0.968750, 0.554687 },
        { -0.179688, -0.968750, 0.554687 },
        { 0.000000, -0.984375, 0.578125 },
        { 0.437500, -0.140625, 0.531250 },
        { -0.437500, -0.140625, 0.531250 },
        { 0.632812, -0.039063, 0.539062 },
        { -0.632812, -0.039063, 0.539062 },
        { 0.828125, 0.148437, 0.445312 },
        { -0.828125, 0.148437, 0.445312 },
        { 0.859375, 0.429687, 0.593750 },
        { -0.859375, 0.429687, 0.593750 },
        ...
        { -0.320312, 0.757812, 0.734375 },
        { 0.156250, 0.718750, 0.757813 },
        { -0.156250, 0.718750, 0.757813 },
        { 0.062500, 0.492187, 0.750000 },
        { -0.062500, 0.492187, 0.750000 },
        { 0.164062, 0.414062, 0.773438 },
        { -0.164062, 0.414062, 0.773438 },
        { 0.125000, 0.304687, 0.765625 },
        { -0.125000, 0.304687, 0.765625 },
        { 0.203125, 0.093750, 0.742188 },
        { -0.203125, 0.093750, 0.742188 },
        ...
        { -0.109375, -0.718750, 0.734375 },
        { 0.117188, -0.835938, 0.710937 },
        { -0.117188, -0.835938, 0.710937 },
        { 0.062500, -0.882813, 0.695312 },
        { -0.062500, -0.882813, 0.695312 },
        { 0.000000, -0.890625, 0.687500 },
        { 0.000000, -0.195313, 0.750000 },
        { 0.000000, -0.140625, 0.742188 },
        { 0.101562, -0.148438, 0.742188 },
        { -0.101562, -0.148438, 0.742188 },
        { 0.125000, -0.226563, 0.750000 },
        { -0.125000, -0.226563, 0.750000 },
        { 0.085938, -0.289063, 0.742188 },
        { -0.085938, -0.289063, 0.742188 },
        { 0.398438, -0.046875, 0.671875 },
        { -0.398438, -0.046875, 0.671875 },
        { 0.617188, 0.054687, 0.625000 },
        { -0.617188, 0.054687, 0.625000 },
        ...
        { -0.312500, 0.640625, 0.835938 },
        { 0.203125, 0.617187, 0.851562 },
        { -0.203125, 0.617187, 0.851562 },
        { 0.101562, 0.429687, 0.843750 },
        { -0.101562, 0.429687, 0.843750 },
        { 0.125000, -0.101563, 0.812500 },
        { -0.125000, -0.101563, 0.812500 },
        { 0.210938, -0.445313, 0.710938 },
        { -0.210938, -0.445313, 0.710938 },
        ...
        { 0.000000, 0.210937, 0.765625 },
        { 0.328125, 0.476562, 0.742188 },
        { -0.328125, 0.476562, 0.742188 },
        { 0.164062, 0.140625, 0.750000 },
        { -0.164062, 0.140625, 0.750000 },
        { 0.132812, 0.210937, 0.757812 },
        { -0.132812, 0.210937, 0.757812 },
        { 0.117188, -0.687500, 0.734375 },
        { -0.117188, -0.687500, 0.734375 },
        { 0.078125, -0.445313, 0.750000 },
        { -0.078125, -0.445313, 0.750000 },
        { 0.000000, -0.445313, 0.750000 },
        { 0.000000, -0.328125, 0.742188 },
        { 0.093750, -0.273438, 0.781250 },
        { -0.093750, -0.273438, 0.781250 },
        { 0.132812, -0.226563, 0.796875 },
        { -0.132812, -0.226563, 0.796875 },
        { 0.109375, -0.132813, 0.781250 },
        { -0.109375, -0.132813, 0.781250 },
        ...
        { -0.093750, -0.156250, 0.812500 },
        { 0.109375, -0.226563, 0.828125 },
        { -0.109375, -0.226563, 0.828125 },
        { 0.078125, -0.250000, 0.804688 },
        { -0.078125, -0.250000, 0.804688 },
        { 0.000000, -0.289063, 0.804688 },
        ...
        { -0.187500, 0.156250, 0.773438 },
        { 0.335938, 0.429687, 0.757812 },
        { -0.335938, 0.429687, 0.757812 },
        { 0.273438, 0.421875, 0.773438 },
        { -0.273438, 0.421875, 0.773438 },
        { 0.421875, 0.398437, 0.773438 },
        { -0.421875, 0.398437, 0.773438 },
        ...
        { -0.242188, 0.125000, 0.757812 },
        { 0.375000, 0.085937, 0.726562 },
        { -0.375000, 0.085937, 0.726562 },
        { 0.460938, 0.117187, 0.703125 },
        { -0.460938, 0.117187, 0.703125 },
        { 0.546875, 0.210937, 0.671875 },
        { -0.546875, 0.210937, 0.671875 },
        { 0.554688, 0.281250, 0.671875 },
        { -0.554688, 0.281250, 0.671875 },
        { 0.531250, 0.335937, 0.679688 },
        { -0.531250, 0.335937, 0.679688 },
        { 0.414062, 0.390625, 0.750000 },
        { -0.414062, 0.390625, 0.750000 },
        { 0.281250, 0.398437, 0.765625 },
        { -0.281250, 0.398437, 0.765625 },
        ...
        { -0.250000, -0.500000, 0.390625 },
        { 0.328125, -0.914062, 0.398437 },
        { -0.328125, -0.914062, 0.398437 },
        { 0.140625, -0.757812, 0.367187 },
        { -0.140625, -0.757812, 0.367187 },
        ...
        { -0.726562, 0.000000, -0.070312 },
        { 0.718750, -0.023437, -0.171875 },
        { -0.718750, -0.023437, -0.171875 },
        { 0.718750, 0.039063, -0.187500 },
        { -0.718750, 0.039063, -0.187500 },
        { 0.796875, 0.203125, -0.210937 },
        { -0.796875, 0.203125, -0.210937 },
        ...
        { -1.312500, 0.054688, -0.531250 },
        { 1.039062, -0.085937, -0.492188 },
        { -1.039062, -0.085937, -0.492188 },
        { 0.789062, -0.125000, -0.328125 },
        { -0.789062, -0.125000, -0.328125 },
        { 0.859375, 0.382813, -0.382812 },
        { -0.859375, 0.382813, -0.382812 }
    };
    ...
    return TestObjFile("suzanne.obj", expectedVertices, expectedFaces);
}

bool TestObjFileWithNonexistentFile()
{
    const char* filename = "DoesNotExist.obj";
    ...
    LogFinishedTesting(filename, success);
    return success;
}